...
describe Decidim::Plans::PlanTagsCell, type: :cell do
  ...
  let(:model) { create(:plan, component: component) }
  ...
  let!(:category_content) { create(:content, :field_category, plan: model, category: category) }
  ...
  context "when rendering" do
    it "renders the form" do
      ...
      expect(subject).to have_link(translated(category.name))
    end
  end
end
