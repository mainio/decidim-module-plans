...
module Decidim
  module Plans
    describe AcceptAccessToPlan do
      ...
      let(:form) { AcceptAccessToPlanForm.from_params(form_params).with_context(current_user: current_user, current_organization: current_organization) }
      let(:form_params) do
        {
          state: state,
          id: id,
          ...
          it "notifies the requester and authors of the plan that access to requester has been accepted" do
            expect(Decidim::EventsManager)
              ...
              .with(
                ...
                resource: plan,
                recipient_ids: plan.authors.pluck(:id) - [requester_user_id],
                extra: {
