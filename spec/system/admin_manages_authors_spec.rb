...
describe "admin maages authors", type: :system do
  include_context "when managing a component"

  let(:component) { create(:plan_component) }
  let(:organization) { component.organization }
  let(:user) { create(:user, :confirmed, :admin, organization: organization) }
  let(:user2) { create(:user, :confirmed, :admin, organization: organization) }
  let!(:plan) { create(:plan, component: component, category: category, users: [user, user2]) }
  let!(:user_group) { create(:user_group, organization: organization) }
  let!(:authors) { create_list(:user, 5, :confirmed, organization: organization) }
  ...
  describe "manage authors" do
    ...
    it "adds authors" do
      ...
      expect(page).to have_css("input#add_plan_authors")
      fill_in "add_plan_authors", with: user_group.name
      expect(page).not_to have_css("#autoComplete_list_1")
      ...
      find("#autoComplete_list_1").click
      click_button "Next"
      expect(page).to have_content "Add authors for proposal"
      expect(page).to have_content "#{author.name} (@#{author.nickname})"
      expect(page).to have_css("button", text: "Add authors")
